...
namespace ASP_NetCore_Aesthetics.Controllers
{
	...
	public class UsersController : ControllerBase
	{
		...
		[HttpPost("CreateAccount_Customer")]
		public async Task<IActionResult> CreateAccount_Customer(User_CreateAccount account)
		{
			try
			{
				//1. Create_Account
				var responseData = await _user.CreateAccount_Customer(account);
				//2. Lưu log request
				...
				//4. Lưu log data user
				_loggerManager.LogInfo("CreateAccount_Customer Response data : " + JsonConvert.SerializeObject(responseData.listUser));
		...
		[HttpPost("CreateAccount_Doctor")]
		public async Task<IActionResult> CreateAccount_Doctor(User_CreateAccount account)
		{
			try
			{
				//1. CreateAccount_Doctor
				var responseData = await _user.CreateAccount_Doctor(account);
				//2. Lưu log request
				_loggerManager.LogInfo("CreateAccount_Doctor Request: " + JsonConvert.SerializeObject(account));
				//3. Lưu log data carts
				_loggerManager.LogInfo("Insert Carts Response data : " + JsonConvert.SerializeObject(responseData.listCarts));
				//4. Lưu log data user
				_loggerManager.LogInfo("Create_Account Response data : " + JsonConvert.SerializeObject(responseData.listUser));
			...
			{
				_loggerManager.LogError("{Error CreateAccount_Doctor} Message: " + ex.Message +
					"|" + "Stack Trace: " + ex.StackTrace);
				return Ok(ex.Message);
			}
		}

		[HttpPost("CreateAccount_Admin")]
		public async Task<IActionResult> CreateAccount_Admin(User_CreateAccount account)
		{
			try
			{
				//1. CreateAccount_Admin
				var responseData = await _user.CreateAccount_Admin(account);
				//2. Lưu log request
				_loggerManager.LogInfo("CreateAccount_Admin Request: " + JsonConvert.SerializeObject(account));
				//3. Lưu log data carts
				_loggerManager.LogInfo("Insert Carts Response data : " + JsonConvert.SerializeObject(responseData.listCarts));
				//4. Lưu log data user
				_loggerManager.LogInfo("Create_Account Response data : " + JsonConvert.SerializeObject(responseData.listUser));
			...
			{
				_loggerManager.LogError("{Error CreateAccount_Admin} Message: " + ex.Message +
					"|" + "Stack Trace: " + ex.StackTrace);
				return Ok(ex.Message);
			}
		}
